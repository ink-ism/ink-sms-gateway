...
|------|------|------|--------|------|
| `id` | BIGINT | PK, AUTO_INCREMENT | - | 主键 |
| `msg_id` | VARCHAR(64) | - | NULL | 客户端消息 ID（唯一业务标识） |
| `server_msg_id` | VARCHAR(32) | - | NULL | CMPP 服务端返回的消息 ID（状态报告匹配用） |
| `sp_id` | VARCHAR(30) | - | NULL | 发送客户标识（REST 发送为 REST，SP 接入为客户标识） |
...
| `status` | TINYINT | - | 0 | 状态：0=已提交, 1=发送成功, 2=发送失败, **3=投递成功(DELIVRD)** |
...
> 记录接收到的短信。列表查询时通过 LEFT JOIN 关联 `ink_sms_down` 表获取最近一条关联下行内容（关联规则：`dest_terminal_id = src_terminal_id` 且 `down.create_time <= up.create_time`，按时间倒序取第一条）。

> **注意**：由于 `ink_sms_up` 使用 `utf8mb4_unicode_ci` 排序规则（建库时指定），而 `ink_sms_down` 使用 MySQL 8.0 默认的 `utf8mb4_0900_ai_ci`，跨表 JOIN 比较时需显式指定 `COLLATE utf8mb4_unicode_ci` 以避免 `Illegal mix of collations` 错误。

| 字段 | 类型 | 约束 | 默认值 | 说明 |
|------|------|------|--------|------|
| `id` | BIGINT | PK, AUTO_INCREMENT | - | 主键 |
| `msg_id` | VARCHAR(64) | - | NULL | CMPP 消息 ID |
| `sp_id` | VARCHAR(30) | - | NULL | **路由目标客户标识** |
...
|------|------|------|--------|------|
| `id` | BIGINT | PK, AUTO_INCREMENT | - | 主键 |
| `sp_id` | VARCHAR(30) | NOT NULL, UNIQUE | - | 客户标识（CMPP Source_Addr，建议≤6字符） |
| `sp_secret` | VARCHAR(100) | NOT NULL | - | 共享密钥（用于 CMPP Connect 认证） |
| `name` | VARCHAR(50) | NOT NULL | - | 客户名称 |
| `status` | TINYINT | - | 1 | 状态：0=禁用，1=启用 |
| `description` | VARCHAR(255) | - | NULL | 客户描述 |
| `create_time` | DATETIME | - | CURRENT_TIMESTAMP | 创建时间 |
| `update_time` | DATETIME | - | CURRENT_TIMESTAMP ON UPDATE | 更新时间 |
...
### ink_sp_channel — 客户通道绑定表

> 存储客户与通道的绑定关系，决定上行/回执消息路由到哪个客户

| 字段 | 类型 | 约束 | 默认值 | 说明 |
|------|------|------|--------|------|
| `id` | BIGINT | PK, AUTO_INCREMENT | - | 主键 |
| `sp_id` | VARCHAR(30) | NOT NULL | - | 客户标识 |
| `channel_code` | VARCHAR(30) | NOT NULL | - | 通道编码 |
| `create_time` | DATETIME | - | CURRENT_TIMESTAMP | 创建时间 |

**唯一约束**：`(sp_id, channel_code)`
...
### ink_blacklist — 通道级退订黑名单表

> 存储通道级退订黑名单，用于拦截已退订号码

| 字段 | 类型 | 约束 | 默认值 | 说明 |
|------|------|------|--------|------|
| `id` | BIGINT | PK, AUTO_INCREMENT | - | 主键 |
| `channel_code` | VARCHAR(30) | NOT NULL | - | 通道编码 |
| `phone` | VARCHAR(21) | NOT NULL | - | 退订手机号 |
| `keyword` | VARCHAR(50) | - | NULL | 触发退订的关键字 |
| `source_mo_id` | VARCHAR(64) | - | NULL | 触发退订的上行消息 ID |
| `expire_time` | DATETIME | NOT NULL | - | 过期时间（过期后自动失效） |
| `create_time` | DATETIME | - | CURRENT_TIMESTAMP | 创建时间 |

**唯一约束**：`(channel_code, phone)`
...
### ink_push_queue — 下游离线推送队列表

> 当 SP 客户端离线时，将上行/回执消息暂存至此表，SP 上线后补发

| 字段 | 类型 | 约束 | 默认值 | 说明 |
|------|------|------|--------|------|
| `id` | BIGINT | PK, AUTO_INCREMENT | - | 主键 |
| `sp_id` | VARCHAR(30) | NOT NULL | - | 目标客户标识 |
| `msg_type` | VARCHAR(10) | NOT NULL | - | 消息类型：REPORT=状态报告, MO=上行短信 |
| `phone` | VARCHAR(21) | - | NULL | 相关手机号 |
| `content` | TEXT | - | NULL | 上行内容（MO） |
| `server_msg_id` | VARCHAR(32) | - | NULL | 原下行服务端消息 ID（REPORT） |
| `stat` | VARCHAR(20) | - | NULL | 状态报告结果（REPORT） |
| `msg_fmt` | INT | - | 8 | 消息格式（MO） |
| `status` | TINYINT | - | 0 | 状态：0=待推送, 1=已推送 |
